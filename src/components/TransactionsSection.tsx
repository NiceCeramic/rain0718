      ...
      {rentals.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {rentals.map((rental) => {
            ...
            // 현재 사용자가 물건의 소유주(위탁자)인지, 대여자(빌린 사람)인지 판별
            // item.owner_id가 없거나 다를 경우 기본적으로 rental.user_id와의 비교로 판별
            const isOwner = item && item.owner_id && currentUser.id 
              ? String(item.owner_id) === String(currentUser.id) 
              : String(rental.user_id) !== String(currentUser.id);

            const isPending = rental.status === 'pending_deposit';
            const isActive = rental.status === 'active';
            const isReturned = rental.status === 'returned';

            const itemPrice = item?.price || rental.price_paid || 10000; 
            ...
            const borrowerRefund = depositPrice; // 대여자 반환 보증금
            ...
            return (
              ...
              >
                {/* Header Badge Row: 역할에 따라 '나눔증' 혹은 '대여증'으로 표시 */}
                ...
                <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs">
                  ...
                  {/* Pricing Info */}
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 min-w-[210px] space-y-1.5 text-slate-600">
                    <div className="flex justify-between text-[11px]">
                      <span>대여 물품 가격 (대여료)</span>
                      <span className="font-bold text-slate-800">₩{itemPrice.toLocaleString()}</span>
                    </div>
                ...
                {/* Status-specific Action Cards (빌리는 사람 vs 빌려주는 사람 분기) */}
                {isPending && !isOwner && (
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-amber-200">
                      <div className="space-y-1">
                        ...
                        <p className="text-slate-600 text-[11px]">
                          대여료 및 보증금 포함 총 <strong className="text-teal-800">₩{totalTransferAmount.toLocaleString()}</strong>을 위 계좌로 송금해주세요.
                        </p>
                      </div>
                  ...
                  <div className="px-5 py-3.5 bg-slate-50 border-t border-slate-200 text-[11px] text-slate-600 space-y-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1 text-[11px]">
                      ...
                      <div className="bg-white p-2 rounded-xl border border-slate-200">
                        ...
                        <strong className="text-teal-700">₩{borrowerRefund.toLocaleString()}</strong>
                      </div>
                      <div className="bg-white p-2 rounded-xl border border-slate-200">
